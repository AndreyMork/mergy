...
import { readParseAndMerge } from '../src';
...
const getTestFiles = (relativePathToTestFilesDir: string): $ReadOnlyArray<string> => {
  ...
  return fs
    .readdirSync(pathToTestFilesDir)
    .map((filepath: string): string => path.join(pathToTestFilesDir, filepath));
};

const pathToTargetsDir: string = path.join(__dirname, '__fixtures__', 'targets');
const getTargetFile = (filename: string): string =>
  fs.readFileSync(path.join(pathToTargetsDir, filename), 'utf-8');

const removeSpaceCharacters = (str: string): string =>
  [...str].filter((char: string): boolean => char.trim().length !== 0).join('');

it('merge configs', async () => {
  const testFiles: $ReadOnlyArray<string> = getTestFiles('mergeConfigs');
  const jsonRes: string = await readParseAndMerge(testFiles, { format: 'json' });
  const yamlRes: string = await readParseAndMerge(testFiles, { format: 'yaml' });
